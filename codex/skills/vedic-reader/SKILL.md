...
```
默认优先级：calculator > PDF/截图/文本提取

只要能获得出生日期、时间、地点：
  1. 必须先运行 vedic-calculator，生成 canonical structured_data.md
  2. PDF/截图/文本中的行星、分盘、Dasha、SAV/BAV、宫主、尊贵度等只做交叉验证
  3. 非Shadbala字段即使与calc不一致，也不得直接覆盖calc；记录偏差并检查出生信息/Ayanamsa/时区
...
通道A【必做】：PyMuPDF提取文本层
  1. 运行PyMuPDF提取全部页面文本
  2. 将文本保存为临时文件
  3. 用Codex文件读取工具查看提取结果（不要用print，终端中文可能乱码）
...
通道B【必做】：AI视觉识别（D1 + D9）
  ⚠️⚠️⚠️ 禁止用浏览器打开PDF！
  视觉识别的正确方法：用PyMuPDF将PDF页面渲染为PNG图片，然后用Codex图片查看工具查看图片。

  1. 用PyMuPDF将包含盘图的页面渲染为PNG（见下方代码）
  2. 用Codex图片查看工具查看渲染出的PNG图片
  3. 识别图表类型（南印/北印）
  4. 读取D1和D9的行星位置
...
with open('extracted_text.txt', 'w', encoding='utf-8') as f:
    f.write(text)
# 用Codex文件读取工具查看 extracted_text.txt

# 通道B：渲染页面为PNG图片（不要用浏览器！）
for i, page in enumerate(doc):
    pix = page.get_pixmap(dpi=200)
    pix.save(f'page_{i}.png')
# 用Codex图片查看工具查看 page_0.png 等图片，进行视觉识别
```
...
⚠️ **南印/北印盘读取规则 → 必须用Codex文件读取工具读取 `resources/chart_reading_rules.md`**
