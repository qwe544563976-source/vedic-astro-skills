...
**第1条构造：身体标记**
```
数据源：Mars落宫 + Mars所在Nakshatra

参考框架（区域级）：
  Mars在1宫->头部/面部  Mars在2宫->面部/口腔/眼部
  Mars在3宫->手臂/肩部  Mars在4宫->胸部/心脏
  Mars在5宫->上腹部/胃  Mars在6宫->腰腹部/消化系统
  Mars在7宫->下腹部/腰  Mars在8宫->生殖器/隐私部位
  Mars在9宫->臀部/大腿  Mars在10宫->膝盖/关节
  Mars在11宫->小腿/脚踝 Mars在12宫->足部/脚趾/眼部

以上是参考框架，不是死查表。结合以下因素自行推理：
  - Mars所在星座阴阳性（阴性可能偏左，阳性偏右，但不确定）
  - Mars度数和Nakshatra
  - Mars的相位关系
  - 用户性别
保持区域级别，多因素一致指向某侧时可说"可能偏左/右侧"。

Nakshatra辅助（痕迹类型）：
  火性星宿（Krittika/Ardra/Magha等）-> 切割型（伤疤、刮痕）
  水性星宿（Rohini/Pushya/Anuradha等）-> 色素型（胎记、痣）
  风性星宿（Ashwini/Swati/Shatabhisha等）-> 淡色/不明显标记
```

**第2条构造：家庭结构 + 父母关系**
```
数据源：
  3宫主+3宫行星+3宫SAV -> 兄弟姐妹数量和排行
...
推导参考：
  9宫主受Saturn/Rahu严重影响 -> "您和父亲的关系有距离感"
  Sun燃烧/落陷/在6-8-12宫 -> "父亲角色在您成长中有某种缺席或限制"
  Moon受克+4宫受损 -> "母亲的情绪或健康曾让您担忧"
  3宫空+3宫主弱 -> "您可能是独生子女或兄弟姐妹较少"
  3宫有多星+3宫主强 -> "您有2个以上的兄弟姐妹"
```
...
数据源：
  6宫主身份+凶星相位 -> 本人慢性问题类型
  2宫+Venus受克 -> 视力/牙齿
  Sun严重受损（燃烧/落陷/6-8-12） -> 父亲健康问题
  Moon严重受损 -> 母亲健康/情绪问题

推导参考（按6宫主判断问题类型）：
  6宫主=Mars -> 炎症/手术/外伤类
  6宫主=Saturn -> 慢性/骨骼/关节/皮肤类
  6宫主=Mercury -> 神经/呼吸/消化类
  6宫+Rahu -> 难以诊断的/罕见的/过敏类

选择策略：选最明显的1项（本人或父母，哪个信号最强选哪个）
```
...
**验前事构造SOP（每次生成前必须走）**
```
步骤1：查Mars落宫+Nakshatra -> 确定身体标记区域+类型
步骤2：查3宫/9宫/4宫+Sun/Moon -> 选最强信号的家庭/父母推断
步骤3：查6宫主/2宫+Venus/Sun/Moon受损 -> 选最明显的健康项
步骤4：列所有已过去的Antardasha -> 选信号最强的2个时间节点
步骤5：检查5条是否全部为可证伪事实，无性格描述 -> 确认后输出
